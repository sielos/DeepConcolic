  ...
  if criterion=='nc':                   ## neuron cover
    ...
    if norm=='linf':
      ...
      engine = nc_setup (test_object = test_object,
        ...
        from nc_l0 import NcL1Analyzer
        l0_args = copy.copy(norm_args)
        del l0_args['LB_noise']
        engine = nc_setup(test_object = test_object,
                          engine_args = engine_args,
                          setup_analyzer = NcL1Analyzer,
                          input_shape = test_object.raw_data.data[0].shape,
                          eval_batch=eval_batch_func(test_object.dnn),
                          postproc_inputs=postproc_inputs,
                          l0_args=l0_args)
    elif norm == 'fro':
        from nc_l0 import NcFroAnalyzer
        l0_args = copy.copy(norm_args)
        del l0_args['LB_noise']
        engine = nc_setup(test_object=test_object,
                          engine_args=engine_args,
                          setup_analyzer=NcFroAnalyzer,
                          input_shape=test_object.raw_data.data[0].shape,
                          eval_batch=eval_batch_func(test_object.dnn),
                          postproc_inputs=postproc_inputs,
                          l0_args=l0_args)
    elif norm == 'nuc':
        from nc_l0 import NcNucAnalyzer
        l0_args = copy.copy(norm_args)
        del l0_args['LB_noise']
        engine = nc_setup(test_object=test_object,
                          engine_args=engine_args,
                          setup_analyzer=NcNucAnalyzer,
                          input_shape=test_object.raw_data.data[0].shape,
                          eval_batch=eval_batch_func(test_object.dnn),
                          postproc_inputs=postproc_inputs,
                          l0_args=l0_args)
    ...
    if norm == 'linf':
      ...
      from dbnc_l0 import BFcL1Analyzer
      l0_args = copy.copy (norm_args)
      del l0_args['LB_noise']
      engine = dbnc_setup (**dbnc_spec,
                           test_object = test_object,
      ...
      from dbnc_l0 import BFcFroAnalyzer
      l0_args = copy.copy (norm_args)
      del l0_args['LB_noise']
      engine = dbnc_setup (**dbnc_spec,
                           test_object = test_object,
      ...
      from dbnc_l0 import BFcNucAnalyzer
      l0_args = copy.copy (norm_args)
      del l0_args['LB_noise']
      engine = dbnc_setup (**dbnc_spec,
                           test_object = test_object,
    ...
    if norm =='linf':
        ...
        from ssc import SScGANBasedAnalyzerL0, setup as ssc_setup
        linf_args = copy.copy (norm_args)
        del linf_args['LB_hard']
        del linf_args['LB_noise']
        engine = ssc_setup (test_object = test_object,
                            engine_args = engine_args,
                            setup_analyzer = SScGANBasedAnalyzerL0,
                            ref_data = test_object.raw_data,
                            input_bounds = input_bounds,
                            postproc_inputs = postproc_inputs,
                            linf_args = linf_args)
        ...
        from ssc import SScGANBasedAnalyzerL1, setup as ssc_setup
        linf_args = copy.copy (norm_args)
        del linf_args['LB_hard']
        del linf_args['LB_noise']
        engine = ssc_setup (test_object = test_object,
                            engine_args = engine_args,
                            setup_analyzer = SScGANBasedAnalyzerL1,
                            ref_data = test_object.raw_data,
                            input_bounds = input_bounds,
                            postproc_inputs = postproc_inputs,
                            linf_args = linf_args)
    elif norm == 'fro':
        from ssc import SScGANBasedAnalyzerFro, setup as ssc_setup
        linf_args = copy.copy (norm_args)
        del linf_args['LB_hard']
        del linf_args['LB_noise']
        engine = ssc_setup (test_object = test_object,
                            engine_args = engine_args,
                            setup_analyzer = SScGANBasedAnalyzerFro,
                            ref_data = test_object.raw_data,
                            input_bounds = input_bounds,
                            postproc_inputs = postproc_inputs,
                            linf_args = linf_args)
    elif norm == 'nuc':
        from ssc import SScGANBasedAnalyzerNuc, setup as ssc_setup
        linf_args = copy.copy (norm_args)
        del linf_args['LB_hard']
        del linf_args['LB_noise']
        engine = ssc_setup (test_object = test_object,
                            engine_args = engine_args,
                            setup_analyzer = SScGANBasedAnalyzerNuc,
                            ref_data = test_object.raw_data,
                            input_bounds = input_bounds,
                            postproc_inputs = postproc_inputs,
                            linf_args = linf_args)
...
def main():
  ...
  parser.add_argument("--norm", dest="norm", required = True, choices = ('l0', 'l1', 'fro', 'nuc', 'linf'),
